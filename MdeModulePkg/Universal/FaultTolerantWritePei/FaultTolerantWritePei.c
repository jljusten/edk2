/** @file
  This driver installs gFaultTolerantWriteGuid PPI to inform
  ...
  )
{
  ...
  //
  // Check signature with gWorkingBlockSignatureGuid
  //
  if (!CompareGuid (&gWorkingBlockSignatureGuid, &WorkingHeader->Signature)) {
    DEBUG ((EFI_D_ERROR, "FtwPei: Work block header signature check error, it should be gWorkingBlockSignatureGuid\n"));
    //
  ...
  )
{
  ...
  if (IsValidWorkSpace (FtwWorkingBlockHeader, WorkSpaceLength)) {
    ...
    if (!EFI_ERROR (Status) && ((FtwLastWriteRecord->SpareComplete == FTW_VALID_STATE) && (FtwLastWriteRecord->DestinationComplete != FTW_VALID_STATE))) {
      ...
      BuildGuidDataHob (&gFaultTolerantWriteGuid, (VOID *) &FtwLastWrite, sizeof (FAULT_TOLERANT_WRITE_LAST_WRITE_DATA));
    }
  } else {
    FtwWorkingBlockHeader = NULL;
    //
    ...
    while (WorkSpaceInSpareArea >= SpareAreaAddress) {
      if (CompareGuid (&gWorkingBlockSignatureGuid, (EFI_GUID *) (UINTN) WorkSpaceInSpareArea)) {
        //
        // Found the workspace.
        //
  ...
  //
  // Install gFaultTolerantWriteGuid PPI to inform the check for FTW last write data has been done.
  //
