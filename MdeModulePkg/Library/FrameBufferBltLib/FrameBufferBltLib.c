  ...
  )
{
  ...
  if (*ConfigSize < sizeof (FRAMEBUFFER_CONFIG)) {
    *ConfigSize = sizeof (FRAMEBUFFER_CONFIG);
  ...
  case PixelRedGreenBlueReserved8BitPerColor:
    ConfigurePixelBitMaskFormat (FbConfig, &RgbPixelMasks);
    break;
  case PixelBlueGreenRedReserved8BitPerColor:
    ConfigurePixelBitMaskFormat (FbConfig, &BgrPixelMasks);
    break;
  case PixelBitMask:
    ConfigurePixelBitMaskFormat (
      FbConfig,
  ...
  FbConfig->PixelFormat = FrameBufferInfo->PixelFormat;

  FbConfig->FrameBuffer = (UINT8*) FrameBuffer;
  FbConfig->WidthInPixels = (UINTN) FrameBufferInfo->HorizontalResolution;
  FbConfig->Height = (UINTN) FrameBufferInfo->VerticalResolution;
  FbConfig->WidthInBytes = FbConfig->WidthInPixels * FbConfig->BytesPerPixel;

  ASSERT (FbConfig->WidthInBytes < sizeof (FbConfig->LineBuffer));
