...
FatChainLoop:
        ...
        and     ax,0xff8                            ; ax = ax & 0xff8
        cmp     ax,0xff8                            ; See if this is the last cluster
...
EvenFatEntry:
        and     bx,0xfff                            ; Strip upper 4 bits of NextClusterNumber
        pop     si                                  ; Restore si
...
;        mov es,ax
    ...
    db    0x66
    db    0xea                        ; jmp far 16:32
OffsetIn32BitProtectedMode:
    dd    0x0                   ; offset $+8   (In32BitProtectedMode)
    dw    0x10                         ; selector  (flat CS)
...
OffsetInLongMode:
    ...
    dw      0x38                ;   Selector is our code selector, 38h
...
ASM_PFX(GDT_BASE):
; null descriptor
%define NULL_SEL $-ASM_PFX(GDT_BASE)          ; Selector [0x0]
        dw 0            ; limit 15:0
        dw 0            ; base 15:0
        db 0            ; base 23:16
        db 0            ; type
...
%define LINEAR_SEL $-ASM_PFX(GDT_BASE)          ; Selector [0x8]
...
%define LINEAR_CODE_SEL $-ASM_PFX(GDT_BASE)          ; Selector [0x10]
        dw 0xFFFF       ; limit 0xFFFFF
        dw 0            ; base 0
        db 0
...
%define SYS_DATA_SEL $-ASM_PFX(GDT_BASE)          ; Selector [0x18]
        dw 0xFFFF       ; limit 0xFFFFF
        dw 0            ; base 0
        db 0
        db 0x92         ; present, ring 0, data, expand-up, writable
...
%define SYS_CODE_SEL $-ASM_PFX(GDT_BASE)          ; Selector [0x20]
        dw 0xFFFF       ; limit 0xFFFFF
        dw 0            ; base 0
        db 0
        db 0x9A         ; present, ring 0, data, expand-up, writable
...
%define SPARE3_SEL $-ASM_PFX(GDT_BASE)          ; Selector [0x28]
        dw 0            ; limit 0xFFFFF
        dw 0            ; base 0
        db 0
...
; system data segment descriptor
;
%define SYS_DATA64_SEL $-ASM_PFX(GDT_BASE)          ; Selector [0x30]
        dw 0xFFFF       ; limit 0xFFFFF
        dw 0            ; base 0
        db 0
        db 0x92         ; P | DPL [1..2] | 1   | 1   | C | R | A
        db 0xCF         ; G | D   | L    | AVL | Segment [19..16]
        db 0

;
; system code segment descriptor
;
%define SYS_CODE64_SEL $-ASM_PFX(GDT_BASE)          ; Selector [0x38]
        dw 0xFFFF       ; limit 0xFFFFF
        dw 0            ; base 0
        db 0
        db 0x9A         ; P | DPL [1..2] | 1   | 1   | C | R | A
        db 0xAF         ; G | D   | L    | AVL | Segment [19..16]
        db 0

; spare segment descriptor
%define SPARE4_SEL $-ASM_PFX(GDT_BASE)            ; Selector [0x40]
        dw 0            ; limit 0xFFFFF
        dw 0            ; base 0
        db 0
        db 0            ; present, ring 0, data, expand-up, writable
