...
%ifdef BOOT_SECTOR_FAT32
  %define LOADER_FILENAME_PART2 0x30325244   ; "DR20"
  %define LOADER_FILENAME_PART3 0x20202030   ; "0___"
%elifdef BOOT_SECTOR_FAT16
  %define LOADER_FILENAME_PART2 0x36315244   ; "DR16"
  %define LOADER_FILENAME_PART3 0x20202036   ; "6___"
%elifdef BOOT_SECTOR_FAT12
  %define LOADER_FILENAME_PART2 0x20205244   ; "DR__"
  %define LOADER_FILENAME_PART3 0x20202020   ; "____"
%else
  %error "Boot sector must be FAT12, FAT16 or FAT32"
%endif
...
%ifdef BOOT_SECTOR_FAT32
  ; The structure for FAT32 starting at offset 36 of the boot sector.
...
; ****************************************************************************
  ...
  mov   al,byte [bp+NoFats]           ; al = NoFats
...
%ifdef BOOT_SECTOR_FAT32
  ...
  mov   word [bp],ax                  ; Save FirstClusterLBA for later use
...
ProgressUpdate:
  mov     ah, 0x0c
  push    ax
  jmp     AxPushedStatusUpdate
StatusUpdate:
  push ax
AxPushedStatusUpdate:
  mov  ax,0xb800
  mov  es,ax
  pop  ax
  mov  [es:160 + 10], ax
